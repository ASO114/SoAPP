...
public final class JceInputStream {
	...
	private <K, V> Map<K, V> readMap(Map<K, V> mr, Map<K, V> m, int tag, boolean isRequire) {
		if (m == null || m.isEmpty()) {
			// throw new TafProxyDecodeException("unable to get type of key and
			// value.");
	...
	public List<Object> readList(int tag, boolean isRequire) {
	 ...
	 * // 生成代码时已经往List里面添加了一个元素，纯粹用来作为类型识别，否则java无法识别到List里面放的是什么类型的数据 if (l ==
	 * null || l.isEmpty()) throw new
	 * OneProxyDecodeException("unable to get type of key and value."); return
	 * (ArrayList<T>) readList(new ArrayList<T>(), l.get(0), tag, isRequire); }
	 * 
	 * private <T> List<T> readList(List<T> lr, T mt, int tag, boolean
	 * isRequire) { //if (l == null || l.isEmpty()) // throw new
	 * OneProxyDecodeException("unable to get type of key and value."); //
	 * 生成代码时已经往List里面添加了一个元素，纯粹用来作为类型识别，否则java无法识别到List里面放的是什么类型的数据 //T mt =
	 * l.get(0); if (skipToTag(tag)) { hd.clear(); readHead(h); switch (h.type)
	 * { case JceStruct.LIST: { int size = read(0, 0, true); if (size < 0) throw
	 * new OneProxyDecodeException("size invalid: " + size); for (int i = 0; i <
	 * size; ++i) { T t = read(mt, 0, true); lr.add(t); } break; } case
	 * JceStruct.SIMPLE_LIST: { HeadData hh = new HeadData(); readHead(hh);
	 * if(hh.type != JceStruct.BYTE){ throw new
	 * OneProxyDecodeException("type mismatch, tag: " + tag + ", type: " +
	 * h.type + ", " + hh.type); } int size = read(0, 0, true); if(size < 0)
	 * throw new OneProxyDecodeException("invalid size, tag: " + tag +
	 * ", type: " + h.type + ", " + hh.type + ", size: " + size); byte[] bytes =
	 * new byte[size]; bs.get(bytes); for (int i = 0; i < size; ++i) {
	 * lr.add((T)Byte.valueOf(bytes[i])); } break; } default: throw new
	 * OneProxyDecodeException("type mismatch."); } } else if (isRequire) {
	 * throw new OneProxyDecodeException("require field not exist."); } return
	 * lr; }
	 */
	...
	public <T> Object read(T o, int tag, boolean isRequire) {
		if (o instanceof Byte) {
			...
			return readMap((Map<Object, Object>) o, tag, isRequire);
		} else if (o instanceof List) {
			return readArray((List<Object>) o, tag, isRequire);
